...
function main(argv) {
  ...
  // `spawn("vite")` cannot launch npm's `.cmd` shim on Windows unless a shell
  // is involved. Keep the command shell-free and invoke Vite's JS entry point
  // with the current Node runtime instead.
  const windowsVite = process.platform === "win32" && command === "vite";
  const child = windowsVite
